...
import { BarChart3, Calendar, Database, MapPin, Sparkles, TrendingUp } from 'lucide-react'
...
export default function ForecastingWorkspace() {
  ...
  const sourceRowCount = dataset?.row_count || 0
  ...
  return (
    <div className="fc-workspace">
      ...
      <div className="fc-map-card">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', padding: '10px 12px 0', color: '#334155', fontSize: '0.8rem' }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '6px 10px', borderRadius: '999px', background: dataset?.source === 'supabase' ? '#dcfce7' : '#fef3c7', color: dataset?.source === 'supabase' ? '#166534' : '#92400e', fontWeight: 700 }}>
            <Database size={14} />
            {activeSource}
          </span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
            <Calendar size={13} />
            {dateRange.start ? `${String(dateRange.start).slice(0, 10)} → ${String(dateRange.end).slice(0, 10)}` : 'Date range unavailable'}
          </span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
            <BarChart3 size={13} />
